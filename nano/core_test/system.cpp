...
TEST (system, work_generate_limited)
{
	...
	auto min = nano::dev::network_params.work.get_entry ();
	auto max = nano::dev::network_params.work.get_base ();
	for (int i = 0; i < 5; ++i)
	{
...
TEST (system, DISABLED_generate_send_existing)
{
	...
	auto send_block (node1.wallets.send_action (wallet_id1, nano::dev::genesis_key.pub, stake_preserver.pub, nano::dev::constants.genesis_amount / 3 * 2, true));
	auto info1 = node1.ledger.any ().account_get (*node1.store.tx_begin_read (), nano::dev::genesis_key.pub);
	ASSERT_TRUE (info1);
	...
	auto info2 = node1.ledger.any ().account_get (*node1.store.tx_begin_read (), nano::dev::genesis_key.pub);
	ASSERT_TRUE (info2);
...
TEST (system, DISABLED_generate_send_new)
{
	...
	nano::keypair stake_preserver;
	auto send_block (node1.wallets.send_action (wallet_id, nano::dev::genesis_key.pub, stake_preserver.pub, nano::dev::constants.genesis_amount / 3 * 2, true));
	{
		auto transaction (node1.store.tx_begin_write ());
		nano::block_builder builder;
		auto open_block = builder
	...
	ASSERT_TIMELY (5s, node1.wallets.voting_reps_count () > 0);
	system.generate_send_new (node1, accounts);
	nano::account new_account{};
	{
		std::vector<nano::account> accounts;
		ASSERT_EQ (nano::wallets_error::none, node1.wallets.get_accounts (wallet_id, accounts));
