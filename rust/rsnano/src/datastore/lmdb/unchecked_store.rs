...
use crate::{
    datastore::{DbIterator, UncheckedStore},
...
impl UncheckedStore<LmdbReadTransaction, LmdbWriteTransaction> for LmdbUncheckedStore {
    fn clear(&self, txn: &LmdbWriteTransaction) {
        let status = unsafe { mdb_drop(txn.handle, self.db_handle(), 0) };
        assert_success(status);
    }

    fn put(&self, txn: &LmdbWriteTransaction, dependency: &HashOrAccount, info: &UncheckedInfo) {
    ...
    fn del(&self, txn: &LmdbWriteTransaction, key: &UncheckedKey) {
        let key_bytes = key.to_bytes();
        let status = unsafe {
            mdb_del(
                txn.handle,
    ...
    fn begin(&self, txn: &Transaction) -> Box<dyn DbIterator<UncheckedKey, UncheckedInfo>> {
