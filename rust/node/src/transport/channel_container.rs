use super::{ChannelDirection, ChannelId, ChannelMode, ChannelTcp};
...
pub struct ChannelContainer {
    by_endpoint: HashMap<SocketAddrV6, Arc<ChannelTcp>>,
...
impl ChannelContainer {
    pub const ELEMENT_SIZE: usize = std::mem::size_of::<ChannelTcp>();

    pub fn insert(&mut self, channel: Arc<ChannelTcp>) -> bool {
    ...
    pub fn iter(&self) -> impl Iterator<Item = &Arc<ChannelTcp>> {
        self.by_endpoint.values()
    }

    pub fn iter_by_last_bootstrap_attempt(&self) -> impl Iterator<Item = &Arc<ChannelTcp>> {
        self.by_bootstrap_attempt
            .iter()
    ...
    pub fn remove_by_endpoint(&mut self, endpoint: &SocketAddrV6) -> Option<Arc<ChannelTcp>> {
    ...
    pub fn get_by_remote_addr(&self, remote_addr: &SocketAddrV6) -> Option<&Arc<ChannelTcp>> {
        self.by_endpoint.get(remote_addr)
    }

    pub fn get_by_peering_addr(&self, peering_addr: &SocketAddrV6) -> Option<&Arc<ChannelTcp>> {
        // TODO use a hashmap?
        self.by_endpoint
            .values()
    ...
    pub fn get_by_id(&self, id: ChannelId) -> Option<&Arc<ChannelTcp>> {
        self.by_id.get(&id).and_then(|ep| self.by_endpoint.get(ep))
    }

    pub fn get_by_node_id(&self, node_id: &PublicKey) -> Option<&Arc<ChannelTcp>> {
        self.by_endpoint
            .values()
