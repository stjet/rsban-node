...
void nano::bootstrap_ascending::service::send (std::shared_ptr<nano::transport::channel> channel, async_tag tag)
{
	...
	nano::asc_pull_req request{ network_consts, tag.id, request_payload };
...
void nano::bootstrap_ascending::service::inspect (store::transaction const & tx, nano::block_status const & result, nano::block const & block)
{
	...
	switch (result)
	{
		...
		case nano::block_status::gap_source:
		{
			...
			const auto source = !block.source_field () ? block.link_field ().value ().as_block_hash () : block.source_field ().value ();
...
std::size_t nano::bootstrap_ascending::service::compute_throttle_size () const
{
	...
	std::size_t size_new = config.bootstrap_ascending.throttle_coefficient * std::sqrt (ledger.cache.block_count ());
