...
async fn handle_rpc(
    ...
    let response = match rpc_command {
        ...
        RpcCommand::WalletDestroy(args) => wallet_destroy(rpc_service.node, rpc_service.enable_control, args).await,
        RpcCommand::WalletLock(args) => wallet_lock(rpc_service.node, rpc_service.enable_control, args).await,
        ...
        RpcCommand::Keepalive(arg) => keepalive(rpc_service.node, rpc_service.enable_control, arg.address, arg.port).await,
        ...
        RpcCommand::NanoToRaw(amount_rpc_message) => nano_to_raw(amount_rpc_message.amount).await,
        RpcCommand::RawToNano(amount_rpc_message) => raw_to_nano(amount_rpc_message.amount).await,
        RpcCommand::WalletAddWatch(args) => wallet_add_watch(rpc_service.node, rpc_service.enable_control, args.wallet, args.accounts).await,
        RpcCommand::WalletRepresentative(args) => wallet_representative(rpc_service.node, args.wallet).await,
        RpcCommand::WorkSet(args) => work_set(rpc_service.node, rpc_service.enable_control, args.wallet, args.account, args.work).await,
        RpcCommand::WorkGet(args) => work_get(rpc_service.node, rpc_service.enable_control, args.wallet, args.account).await,
        RpcCommand::WalletWorkGet(args) => wallet_work_get(rpc_service.node, rpc_service.enable_control, args.wallet).await,
        RpcCommand::AccountsFrontiers(args) => accounts_frontiers(rpc_service.node, args.accounts).await,
        ...
        RpcCommand::PasswordChange(args) => password_change(rpc_service.node, rpc_service.enable_control, args.wallet, args.password).await,
        RpcCommand::PasswordEnter(args) => password_enter(rpc_service.node, args.wallet, args.password).await,
        ...
        RpcCommand::WorkCancel(args) => work_cancel(rpc_service.node, rpc_service.enable_control, args).await,
        ...
        RpcCommand::WalletRepresentativeSet(args) => wallet_representative_set(rpc_service.node, rpc_service.enable_control, args).await,
        RpcCommand::SearchReceivable(args) => search_receivable(rpc_service.node, rpc_service.enable_control, args).await,
        RpcCommand::WalletRepublish(args) => wallet_republish(rpc_service.node, rpc_service.enable_control, args).await,
        ...
        RpcCommand::RepresentativesOnline(args) => representatives_online(rpc_service.node, args).await,
