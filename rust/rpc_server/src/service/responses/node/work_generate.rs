...
pub async fn work_generate(node: Arc<Node>, enable_control: bool, args: WorkGenerateArgs) -> String {
    ...
    let max_difficulty = DifficultyV1::from_multiplier(node.config.max_work_generate_multiplier, default_difficulty);
    
    // Validate difficulty
    if difficulty > max_difficulty || 
       difficulty < node.network_params.work.threshold_entry(BlockType::State, work_version) {
       return to_string_pretty(&ErrorDto::new("Difficulty out of valid range".to_string())).unwrap()
    }
    ...
    if let Some(block) = args.block {
        ...
        if args.version.is_some() && work_version != block_enum.work_version() {
            return to_string_pretty(&ErrorDto::new("Block work version mismatch".to_string())).unwrap()
        }
        ...
        if node.network_params.work.difficulty(work_version, &args.hash.into(), 0) >= difficulty {
            return to_string_pretty(&ErrorDto::new("Block work is already sufficient".to_string())).unwrap()
        }
    }
    ...
    if account.is_none() {
        ...
        account = node.ledger.any().block_account(&node.ledger.read_txn(), &args.hash);
    }
    ...
    let work_result = if !use_peers {
        if node.distributed_work.work_generation_enabled() {
            node.distributed_work.make(args.hash.into(),difficulty, account).await
        } else {
            return to_string_pretty(&ErrorDto::new("Local work generation is disabled".to_string())).unwrap()
        }
    } else {
        if node.distributed_work.work_generation_enabled() {
            node.distributed_work.make(args.hash.into(),difficulty, account).await
    ...
    let result_multiplier = DifficultyV1::to_multiplier(result_difficulty, node.ledger.constants.work.threshold_base(work_version));
