...
void nano::json_handler::confirmation_quorum ()
{
	...
	response_l.put ("peers_stake_total", node.rep_crawler.representative_register.total_weight ().convert_to<std::string> ());
	if (request.get<bool> ("peer_details", false))
	{
		boost::property_tree::ptree peers;
		for (auto & peer : node.rep_crawler.representative_register.representatives ())
		{
