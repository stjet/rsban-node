...
if [ -n "$DOCKER_PASSWORD" ]; then
    ...
    if [[ "$TRAVIS_BRANCH" == "master" || "$TRAVIS_BRANCH" == "docker_cache" ]] && [[ "${TRAVIS_BUILD_STAGE_NAME}" =~ 'Build' ]]; then
        ci_image_name="simpago/nano-env:$TRAVIS_JOB_NAME"
    ...
    if [[ "$TRAVIS_BUILD_STAGE_NAME" == "Master_beta_docker" ]]; then
        ...
        "$scripts"/custom-timeout.sh 30 docker build --build-arg NETWORK=beta --build-arg CI_BUILD=true --build-arg TRAVIS_TAG="$TRAVIS_TAG" -f docker/node/Dockerfile -t simpago/nano-beta:master --cache-from simpago/nano-beta:master .
        "$scripts"/custom-timeout.sh 30 docker push simpago/nano-beta:master
        ...
        if [[ "$TRAVIS_JOB_NAME" =~ "live" ]]; then
            ...
            cached="--cache-from=simpago/nano-beta:master"
            docker pull simpago/nano-beta:master
        fi
