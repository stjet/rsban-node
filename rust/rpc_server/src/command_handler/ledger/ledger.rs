    ...
    pub(crate) fn ledger(&self, args: LedgerArgs) -> RpcDto {
        ...
        let account_iter: Box<dyn Iterator<Item = (Account, AccountInfo)>> = match account {
            Some(acc) => Box::new(
                self.node
                    .store
                    .account
                    .iter_range(&block_transaction, acc..),
            ),
            None => Box::new(self.node.store.account.iter(&block_transaction)),
        };
        ...
        if !sorting {
            for (current_account, info) in account_iter {
                if info.modified >= modified_since {
                    let account_receivable = if receivable {
                        self.node.ledger.account_receivable(
                            &block_transaction,
                            &current_account,
                            false,
                        )
            ...
            match account {
                Some(acc) => {
                    let mut iter = self
                        ...
                        .store
                        .account
                        .begin_account(&block_transaction, &acc);
                ...
                None => {
                    let iter = self.node.store.account.iter(&block_transaction);
            ...
            for (_, account) in ledger_l {
                if let Some(info) = self.node.store.account.get(&block_transaction, &account) {
                    let account_receivable = if receivable {
                        self.node
                            .ledger
