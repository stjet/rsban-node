...
pub async fn account_info(
    ...
    if include_confirmed {
        let confirmed_balance = if info.block_count != confirmation_height_info.height {
            node.ledger.any().block_balance(&txn, &confirmation_height_info.frontier).unwrap_or(Amount::zero())
        } else {
            info.balance
        };
    ...
    if args.representative.unwrap_or(false) {
        ...
        if include_confirmed {
            let confirmed_representative = if confirmation_height_info.height > 0 {
                if let Some(confirmed_frontier_block) = node.ledger.any().get_block(&txn, &confirmation_height_info.frontier) {
                    confirmed_frontier_block.representative_field().unwrap_or_else(|| {
                        let rep_block_hash = node.ledger.representative_block_hash(&txn, &confirmation_height_info.frontier);
                        node.ledger.any().get_block(&txn, &rep_block_hash).unwrap().representative_field().unwrap()
                    })
                } else {
                    info.representative
                }
...
mod tests {
    ...
    fn account_info() {
        ...
        let result = node.tokio.block_on(async {
            rpc_client.account_info(*DEV_GENESIS_ACCOUNT,
                Some(true),
                Some(true),
                Some(true),
                Some(true),
