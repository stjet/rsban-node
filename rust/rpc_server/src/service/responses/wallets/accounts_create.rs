...
pub async fn accounts_create(node: Arc<Node>, enable_control: bool, args: AccountsCreateArgs) -> String {
    if !enable_control {
...
mod tests {
    ...
    fn accounts_create_work_true() {
        ...
        let result = node
            .tokio
            .block_on(async { rpc_client.accounts_create(wallet_id, 1, Some(true)).await.unwrap() });

        assert!(node.wallets.exists(&result.accounts[0].into()));

        sleep(Duration::from_millis(10000));

        assert_ne!(node.wallets.work_get2(&wallet_id, &result.accounts[0].into()).unwrap(), 0);
    ...
    fn accounts_create_work_false() {
        ...
        let result = node
            .tokio
            .block_on(async { rpc_client.accounts_create(wallet_id, 1, Some(false)).await.unwrap() });

        assert!(node.wallets.exists(&result.accounts[0].into()));

        sleep(Duration::from_millis(10000));

        assert_eq!(node.wallets.work_get2(&wallet_id, &result.accounts[0].into()).unwrap(), 0);
