...
nano::shared_const_buffer::shared_const_buffer (const uint8_t * data, std::size_t size) :
	m_data (std::make_shared<std::vector<uint8_t>> (data, data + size)),
	m_buffer (boost::asio::buffer (*m_data))
{
}
...
const std::uint8_t * nano::shared_const_buffer::data () const
{
	return m_data->data ();
}
