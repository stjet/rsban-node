...
use crate::{config::NetworkConstants, ffi::config::{NetworkConstantsDto, fill_network_constants_dto}, ipc::{IpcConfig, IpcConfigTransport}};
...
pub unsafe extern "C" fn rsn_ipc_config_create(
    ...
    let dto = &mut (*dto);
    fill_config_transport_dto(&mut dto.domain_transport, &config.transport_domain.transport);
    let bytes = config.transport_domain.path.as_os_str().as_bytes();
    if bytes.len() > dto.domain_path.len() { return -1;}
    ...
    fill_network_constants_dto(&mut dto.tcp_network_constants, &config.transport_tcp.network_constants);
    dto.tcp_port = config.transport_tcp.port;
    dto.flatbuffers_skip_unexpected_fields_in_json = config.flatbuffers.skip_unexpected_fields_in_json;
