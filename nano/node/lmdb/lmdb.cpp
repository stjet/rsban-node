...
void nano::lmdb::store::upgrade_v14_to_v15 (nano::write_transaction & transaction_a)
{
	...
	for (; i_account != n_account; ++i_account)
	{
		...
		mdb_put (env ().tx (transaction_a), confirmation_height_store.table_handle (), nano::mdb_val (account), nano::mdb_val (account_info_v14.confirmation_height), MDB_APPEND);
...
void nano::lmdb::store::upgrade_v16_to_v17 (nano::write_transaction const & transaction_a)
{
	...
	auto num = 0u;
	for (nano::mdb_iterator<nano::account, uint64_t> i (transaction_a, confirmation_height_store.table_handle ()), n (nano::mdb_iterator<nano::account, uint64_t>{}); i != n; ++i, ++account_info_i, ++num)
	{
	...
	auto status (mdb_drop (env ().tx (transaction_a), confirmation_height_store.table_handle (), 0));
