...
mod tests {
    ...
    fn serialize_block_info_dto() {
        let block_info = BlockInfoDto {
            block_account: Account::decode_account("nano_1ipx847tk8o46pwxt5qjdbncjqcbwcc1rrmqnkztrfjy5k7z4imsrata9est").unwrap(),
            amount: Amount::raw(30000000000000000000000000000000000u128),
            balance: Amount::raw(5606157000000000000000000000000000000u128),
            height: 58,
            local_timestamp: 0,
            successor: BlockHash::decode_hex("8D3AB98B301224253750D448B4BD997132400CEDD0A8432F775724F2D9821C72").unwrap(),
            confirmed: true,
    ...
    fn deserialize_block_info_dto() {
        ...
        assert_eq!(deserialized.block_account, Account::decode_account("nano_1ipx847tk8o46pwxt5qjdbncjqcbwcc1rrmqnkztrfjy5k7z4imsrata9est").unwrap());
        assert_eq!(deserialized.amount, Amount::raw(30000000000000000000000000000000000u128));
        assert_eq!(deserialized.balance, Amount::raw(5606157000000000000000000000000000000u128));
        assert_eq!(deserialized.height, 58);
        assert_eq!(deserialized.local_timestamp, 0);
        assert_eq!(deserialized.successor.to_string(), "8D3AB98B301224253750D448B4BD997132400CEDD0A8432F775724F2D9821C72");
        assert!(deserialized.confirmed);
        assert_eq!(deserialized.subtype, BlockSubType::Send);
        assert_eq!(deserialized.contents, BlockEnum::new_test_instance().json_representation());
    }
}