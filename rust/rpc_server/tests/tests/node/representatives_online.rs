...
fn representatives_online() {
    ...
    assert_eq!(filtered_result.accounts.len(), 1);
    assert!(filtered_result.accounts.contains_key(&new_rep.into()));
    assert!(!filtered_result.accounts.contains_key(&(*DEV_GENESIS_ACCOUNT)));
