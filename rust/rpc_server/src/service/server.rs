...
async fn handle_rpc(
    ...
    let response = match rpc_command {
        ...
        RpcCommand::WorkCancel(args) => work_cancel(rpc_service.node, rpc_service.enable_control, args.hash).await,
        RpcCommand::Bootstrap(bootstrap_args) => bootstrap(rpc_service.node, bootstrap_args.address, bootstrap_args.port, bootstrap_args.id).await,
        ...
        RpcCommand::UncheckedGet(args) => unchecked_get(rpc_service.node, args.hash).await,
