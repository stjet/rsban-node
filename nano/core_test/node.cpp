...
TEST (node, rollback_vote_self)
{
	...
	auto fork = builder.make_block ()
				.from (*send2)
				.balance (send1->balance ().value ().number () - 2)
				.sign (nano::dev::genesis_key.prv, nano::dev::genesis_key.pub)
				.build ();
...
TEST (node, dependency_graph)
{
	...
	auto gen_send2 = builder.make_block ()
					 ...
					 .link (key2.pub)
					 .balance (gen_receive->balance ().value ().number () - 2)
					 .sign (nano::dev::genesis_key.prv, nano::dev::genesis_key.pub)
	...
	auto key2_send2 = builder.make_block ()
					  ...
					  .link (key1.pub)
					  .balance (key2_send1->balance ().value ().number () - 1)
					  .sign (key2.prv, key2.pub)
	...
	auto key1_receive = builder.make_block ()
						...
						.balance (key1_send1->balance ().value ().number () + 1)
						.sign (key1.prv, key1.pub)
	...
	auto key1_send2 = builder.make_block ()
					  ...
					  .link (key3.pub)
					  .balance (key1_receive->balance ().value ().number () - 1)
					  .sign (key1.prv, key1.pub)
	...
	auto key3_receive = builder.make_block ()
						...
						.balance (key3_open->balance ().value ().number () + 1)
						.sign (key3.prv, key3.pub)
	...
	auto key3_epoch = builder.make_block ()
					  ...
					  .balance (key3_receive->balance ().value ())
					  .sign (nano::dev::genesis_key.prv, nano::dev::genesis_key.pub)
