		...
		if (!error)
		{
			...
			switch (header.get_type ())
			{
				...
				case nano::message_type::keepalive:
				{
					get_socket ()->async_read (get_buffer (), header.payload_length_bytes (), [this_l, header] (boost::system::error_code const & ec, std::size_t size_a) {
						auto ec_dto{ rsnano::error_code_to_dto (ec) };
						rsnano::rsn_bootstrap_server_receive_keepalive_action (this_l->handle, &ec_dto, size_a, header.handle);
					});
					break;
				}
				...
				case nano::message_type::confirm_ack:
				{
					get_socket ()->async_read (get_buffer (), header.payload_length_bytes (), [this_l, header] (boost::system::error_code const & ec, std::size_t size_a) {
						auto ec_dto{ rsnano::error_code_to_dto (ec) };
						rsnano::rsn_bootstrap_server_receive_confirm_ack_action (this_l->handle, &ec_dto, size_a, header.handle);
					});
					break;
				}
				...
				case nano::message_type::node_id_handshake:
				{
					get_socket ()->async_read (get_buffer (), header.payload_length_bytes (), [this_l, header] (boost::system::error_code const & ec, std::size_t size_a) {
						auto ec_dto{ rsnano::error_code_to_dto (ec) };
						rsnano::rsn_bootstrap_server_receive_node_id_handshake_action (this_l->handle, &ec_dto, size_a, header.handle);
					});
					break;
				}
