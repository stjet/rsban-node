...
pub async fn republish(node: Arc<Node>, hash: BlockHash, sources: Option<u64>, destinations: Option<u64>, count: Option<u64>) -> String {
    ...
    if let Some(mut block) = node.ledger.any().get_block(&transaction, &hash) {
        ...
        for _ in 0..count {
            ...
            if let Some(sources_count) = sources {
                let source = block.source_field().or_else(|| block.link_field().map(|link| link.into())).unwrap_or_default();
            ...
            if let Some(destinations_count) = destinations {
                if let Some(destination) = block.destination() {
                    if !node.ledger.any().get_pending(&transaction, &PendingKey::new(destination, hash)).is_some() {
                        let mut previous = match node.ledger.any().account_head(&transaction, &destination) {
                            Some(block_hash) => block_hash,
                            None => return to_string_pretty(&ErrorDto::new("Account head not found".to_string())).unwrap()
                        };
                        ...
                        while let Some(db) = dest_block {
                            ...
                            let source = db.source_field().or_else(|| {
                                if db.is_send() {
                                    None
                                } else {
            ...
            let next_hash = node.ledger.any().block_successor(&transaction, &hash).unwrap_or_default();
...
mod tests {
    ...
    fn test_republish_send_block() {
        ...
        let send = node.ledger.any().get_block(&node.store.tx_begin_read(), &node.ledger.any().block_successor(&node.store.tx_begin_read(), &*DEV_GENESIS_HASH).unwrap()).unwrap();

        // Test: Republish send block
        let result = node.tokio.block_on(async {
            rpc_client.republish(send.hash(), None, None, None).await.unwrap()
        });

        assert_eq!(result.blocks.len(), 1, "Expected 1 block, got {}", result.blocks.len());
        assert_eq!(result.blocks[0], send.hash(), "Unexpected block hash");

        assert_timely_msg(
            Duration::from_secs(10),
            || node.ledger.any().block_exists(&node.ledger.read_txn(), &send.hash()),
    ...
    fn test_republish_genesis_block() {
        ...
        let result = node.tokio.block_on(async {
            rpc_client.republish(*DEV_GENESIS_HASH, None, None, Some(1)).await.unwrap()
        });
    ...
    fn test_republish_open_block_with_sources() {
        ...
        let result = node.tokio.block_on(async {
            rpc_client.republish(block_hash, Some(2), None, None).await.unwrap()
        });

        assert_eq!(result.blocks.len(), 3, "Expected 3 blocks, got {}", result.blocks.len());
        assert_eq!(result.blocks[0], *DEV_GENESIS_HASH, "Unexpected genesis block hash");
        assert_eq!(result.blocks[1], node.ledger.any().block_successor(&node.store.tx_begin_read(), &*DEV_GENESIS_HASH).unwrap(), "Unexpected send block hash");
