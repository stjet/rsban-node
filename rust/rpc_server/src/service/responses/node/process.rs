...
use rsnano_rpc_messages::{ProcessArgs, BlockHashRpcMessage, ErrorDto};
    ...
    if !is_async {
        match node.process_local(block.clone()) {
            Some(result) => match result {
                ...
                BlockStatus::GapPrevious => to_string_pretty(&ErrorDto::new("gap previous".to_string())).unwrap(),
                BlockStatus::GapSource => to_string_pretty(&ErrorDto::new("Gap source".to_string())).unwrap(),
                BlockStatus::Old => to_string_pretty(&ErrorDto::new("Old".to_string())).unwrap(),
                BlockStatus::BadSignature => to_string_pretty(&ErrorDto::new("Bad signature".to_string())).unwrap(),
                BlockStatus::NegativeSpend => to_string_pretty(&ErrorDto::new("Negative spend".to_string())).unwrap(),
                BlockStatus::BalanceMismatch => to_string_pretty(&ErrorDto::new("Balance mismatch".to_string())).unwrap(),
                BlockStatus::Unreceivable => to_string_pretty(&ErrorDto::new("Unreceivable".to_string())).unwrap(),
                BlockStatus::BlockPosition => to_string_pretty(&ErrorDto::new("Block position".to_string())).unwrap(),
                BlockStatus::GapEpochOpenPending => to_string_pretty(&ErrorDto::new("Gap epoch open pending".to_string())).unwrap(),
                BlockStatus::Fork => {
                ...
                BlockStatus::InsufficientWork => to_string_pretty(&ErrorDto::new("Insufficient work".to_string())).unwrap(),
                BlockStatus::OpenedBurnAccount => to_string_pretty(&ErrorDto::new("Opened burn account".to_string())).unwrap(),
