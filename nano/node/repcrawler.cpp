...
void nano::rep_crawler::validate ()
{
	...
	for (auto const & i : responses_l)
	{
		...
		{
			node.logger->debug (nano::log::type::rep_crawler, "Ignoring vote from loopback channel: {}", channel->to_string ());
		...
		{
			node.logger->debug (nano::log::type::rep_crawler, "Ignoring vote from account {} with too little voting weight: {}",
		...
		{
			node.logger->info (nano::log::type::rep_crawler, "Found representative {} at {}", vote->account ().to_account (), channel->to_string ());
		}

		if (insert_result.updated)
		{
			node.logger->warn (nano::log::type::rep_crawler, "Updated representative {} at {} (was at: {})", vote->account ().to_account (), channel->to_string (), insert_result.prev_endpoint.address ().to_string ());
		}
	}
}
