			...
			if (!current.is_zero ())
			{
				if (account == current)
				{
					...
					else
					{
						...
						{
							attempt->add_frontier (nano::pull_info (account, latest, frontier, attempt->get_incremental_id (), 0, connection->node->network_params.bootstrap.frontier_retry_limit));
				...
				else
				{
					...
					attempt->add_frontier (nano::pull_info (account, latest, nano::block_hash (0), attempt->get_incremental_id (), 0, connection->node->network_params.bootstrap.frontier_retry_limit));
				}
			}
			else
			{
