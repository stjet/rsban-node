...
bool nano::election_helper::transition_time (nano::confirmation_solicitor & solicitor_a, nano::election & election)
{
	...
	if (!confirmed (election) && election.time_to_live () < std::chrono::milliseconds{ rsnano::rsn_election_elapsed_ms (election.handle) })
	{
		...
		if (!election.state_change (state_l, nano::election::state_t::expired_unconfirmed))
		{
			...
			{
				election.log_votes (guard, tally_impl (guard), "Election expired: ");
			}
...
void nano::election_helper::confirm_if_quorum (nano::election_lock & lock_a, nano::election & election)
{
	...
	if (sum >= node.online_reps.delta () && winner_hash_l != status_winner_hash_l)
	{
		...
		election.remove_votes (lock_a, status_winner_hash_l);
	...
	if (have_quorum (tally_l))
	{
		...
		if (!node.ledger.cache.final_votes_confirmation_canary () || lock_a.final_weight ().number () >= node.online_reps.delta ())
		{
			...
			{
				election.log_votes (lock_a, tally_l);
			}
...
nano::election_vote_result nano::election::vote (nano::election_helper & helper, nano::account const & rep, uint64_t timestamp_a, nano::block_hash const & block_hash_a, vote_source vote_source_a)
{
	...
	{
		helper.confirm_if_quorum (lock, *this);
	}
