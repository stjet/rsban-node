...
private:
	...
	void handle_generate (nano::block_hash const & hash_a)
	{
		...
		{
			auto hash = hash_a;
			auto request_difficulty = work_pool.threshold_base (version);
			work_pool.generate (version, hash, request_difficulty, [this_l = shared_from_this (), hash] (boost::optional<uint64_t> work_a) {
				...
				auto result_difficulty (this_l->work_pool.difficulty (this_l->version, hash, result));
				ptree::ptree message_l;
				message_l.put ("work", nano::to_string_hex (result));
				message_l.put ("difficulty", nano::to_string_hex (result_difficulty));
				message_l.put ("multiplier", nano::to_string (nano::difficulty::to_multiplier (result_difficulty, this_l->work_pool.threshold_base (this_l->version))));
