...
extern "C" {
	...
	bool rsn_token_bucket_try_consume(void* bucket, unsigned tokens_required);
} 

nano::rate::token_bucket::token_bucket (size_t max_token_count_a, size_t refill_rate_a)
{
	bucket_handle = rsn_token_bucket_create();
...
{
	return rsn_token_bucket_try_consume(bucket_handle, tokens_required_a);
}
