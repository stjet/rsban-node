...
TEST (system, block_sequence)
{
	...
	while (!done)
	{
		if (std::chrono::system_clock::now () - last > interval)
		{
			...
			{
				message += boost::str (boost::format ("N:%1% b:%2% c:%3% a:%4% s:%5% p:%6%\n") % std::to_string (i->network->get_port ()) % std::to_string (i->ledger.cache.block_count ()) % std::to_string (i->ledger.cache.cemented_count ()) % std::to_string (i->active.size ()) % std::to_string (i->scheduler.buckets.size ()) % std::to_string (i->network->size ()));
