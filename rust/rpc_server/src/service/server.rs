...
async fn handle_rpc(
    ...
    let response = match rpc_command {
        ...
        RpcCommand::Keepalive(args) => keepalive(rpc_service.node, rpc_service.enable_control, args).await,
        ...
        RpcCommand::WalletAddWatch(args) => wallet_add_watch(rpc_service.node, rpc_service.enable_control, args).await,
        RpcCommand::WalletRepresentative(args) => wallet_representative(rpc_service.node, args).await,
        RpcCommand::WorkSet(args) => work_set(rpc_service.node, rpc_service.enable_control, args).await,
        RpcCommand::WorkGet(args) => work_get(rpc_service.node, rpc_service.enable_control, args).await,
        RpcCommand::WalletWorkGet(args) => wallet_work_get(rpc_service.node, rpc_service.enable_control, args).await,
        ...
        RpcCommand::PasswordChange(args) => password_change(rpc_service.node, rpc_service.enable_control, args).await,
        ...
        RpcCommand::AccountsRepresentatives(args) => accounts_representatives(rpc_service.node, args).await,
        RpcCommand::StatsClear => stats_clear(rpc_service.node).await,
        RpcCommand::UncheckedClear => unchecked_clear(rpc_service.node).await,
        RpcCommand::Unopened(args) => unopened(rpc_service.node, rpc_service.enable_control, args).await,
