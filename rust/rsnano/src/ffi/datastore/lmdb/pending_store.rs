...
use crate::{
    ...
    Account, Amount, BlockHash, Epoch, PendingInfo, PendingKey,
};
...
#[no_mangle]
pub unsafe extern "C" fn rsn_lmdb_pending_store_open_db(
    handle: *mut LmdbPendingStoreHandle,
...
pub unsafe extern "C" fn rsn_lmdb_pending_store_begin_at_key(
    ...
    let mut iterator = (*handle).0.begin_at_key((*txn).as_txn(), &key);
    to_lmdb_iterator_handle(iterator.as_mut())
}
