...
mod tests {
    ...
    use rsnano_core::{validate_block_signature, Account, Amount, BlockEnum, StateBlock, WalletId, DEV_GENESIS_KEY};
    ...
    fn sign() {
        ...
        node.wallets.insert_adhoc2(&wallet_id, &key.private_key(), false);
        ...
        let result = node.tokio.block_on(async {
            rpc_client
                .sign(None, Some(wallet_id), Some(key.public_key().into()), send.json_representation())
                .await
                .unwrap()
        });
