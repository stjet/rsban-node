...
impl RepWeights {
    ...
    pub fn copy_from(&mut self, other: &RepWeights) {
    ...
    pub fn representation_put(&mut self, account: Account, representation: u128) {
        let mut guard = self.rep_amounts.lock().unwrap();
        self.put(&mut guard, account, representation);
    }

    pub fn representation_get(&mut self, account: &Account) -> u128 {
